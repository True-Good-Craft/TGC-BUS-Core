...
| Session/auth authority | Current runtime canonical; DB-backed auth API added | Current runtime remains `core/api/http.py` auth stack, `GET /session/token`, `tgc.security.require_token_ctx`; DB-backed auth routes live in `core/api/routes/auth.py` over `core/auth/*` plus `auth_*` tables | `core.api.http` still owns current runtime-token validation. `/auth/*` supports claimed/unclaimed account lifecycle but is not global runtime authority yet; `/session/token` remains compatibility/bootstrap and is not claimed-mode identity authority. See `04_SECURITY_TRUST_AND_OPERATIONS.md`. |
...
| Session/token split | Narrowed drift with auth API | `core.api.http` is the current canonical validator authority, `AppState.tokens` is the canonical runtime token source, `tgc.security.require_token_ctx` is a compatibility wrapper, and the global/token-file mirrors still participate secondarily. Claimed-mode identity/session rows now exist behind `/auth/*`, but `/session/token` remains compatibility only and must not bypass login once claimed-mode cutover happens. | `04_SECURITY_TRUST_AND_OPERATIONS.md` |
