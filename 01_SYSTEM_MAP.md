...
| Session/auth authority | Current runtime canonical; auth schema/helper skeleton present | Current runtime remains `core/api/http.py` auth stack, `GET /session/token`, `tgc.security.require_token_ctx`; future identity authority lives under `core/auth/*` plus DB-backed `auth_*` tables | `core.api.http` owns current runtime-token validation. The `core/auth/*` helper skeleton and auth tables are not wired into login/session runtime yet; `/session/token` remains compatibility/bootstrap and is not final claimed-mode identity authority. See `04_SECURITY_TRUST_AND_OPERATIONS.md`. |
...
| Session/token split | Narrowed drift with auth skeleton | `core.api.http` is the current canonical validator authority, `AppState.tokens` is the canonical runtime token source, `tgc.security.require_token_ctx` is a compatibility wrapper, and the global/token-file mirrors still participate secondarily. Future claimed-mode identity authority is scaffolded under `core/auth/*`; `/session/token` is compatibility only and must not bypass login once claimed. | `04_SECURITY_TRUST_AND_OPERATIONS.md` |
