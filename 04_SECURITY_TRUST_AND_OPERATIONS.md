...
| Claimed owner identity model | Global gate, permissions, and backend management implemented | `core/api/http.py`, `core/api/routes/auth.py`, `core/api/routes/users.py`, `core/auth/dependencies.py`, `core/auth/management.py`, `core/appdb/models_auth.py`, low-level `core/auth/*` helpers | User accounts, roles, permissions, sessions, recovery-code hashes, and audit events | `/auth/state`, `/auth/setup-owner`, `/auth/login`, `/auth/logout`, `/auth/me`, `/app/users`, `/app/roles`, `/app/sessions`, and `/app/audit` exist. Once one or more users exist, protected routes require `bus_auth_session`; legacy `bus_session` no longer grants `/app/*` access. Covered routes enforce route-local permissions. UI remains deferred. |
...
Phase 3 implemented the global claimed-mode gate. Phase 4 implemented route-local permission checks for covered protected route families. Phase 5 implements backend-only user, role, session, and audit management routes. It does not add UI.
