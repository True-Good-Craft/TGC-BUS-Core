...
- Narrowed drift: `/app/update/check` still only surfaces `download_url`, but internal helpers now hash-verify cached release ZIPs against signed manifest metadata, safely extract them into the local update cache, verify the extracted EXE with Authenticode plus pinned signer policy, and promote `verified_ready` conservatively without adding launch or handoff behavior.
